...
/// insère [`AudioBackendHandle`] ; sans cette ressource, tous les systèmes de la crate, qui
/// l'empruntent en `ResMut`, seraient écartés en silence.
///
...
impl GameAudioPlugin {
    /// Le backend réel, sur la sortie audio de la machine ; muet s'il n'y en a pas. Exige un
    /// `AssetPlugin` déjà monté.
    pub fn new() -> Self {
        Self {
    ...
    /// lui-même. Voie des tests sur le son réel. Exige un `AssetPlugin` déjà monté.
    pub fn offline() -> Self {
        Self {
